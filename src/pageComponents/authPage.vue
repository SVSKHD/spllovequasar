...
// Palette (never empty)
const palette = (
  props.colors?.length ? props.colors : ['#e91e63', '#ff1744', '#f50057', '#d500f9', '#ff6d00']
) as readonly string[];
...
const userData = ref<{ login?: boolean; spoorthy?: boolean; hithesh?: boolean; time?: number }>({});

// Check login validity on mount
onMounted(() => {
  ...
  if (loginStr) {
    try {
      ...
      const now = Date.now();
      if (!loginObj.time || now - loginObj.time > 30 * 60 * 1000) {
...
.love-page {
  ...
  overflow: hidden;
  background:
    radial-gradient(1200px 600px at 50% 70%, rgba(255, 0, 102, 0.06), transparent 60%), #fff;
}
