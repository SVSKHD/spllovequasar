<template>
  <div v-if="timelineView" class="text-center">
    <h1 class="text-title">
      Well We have been creating memories and i wanted to record every memory and see the roll back
    </h1>
    <q-btn
      class="glass-btn love-btn text-center"
      @click="timelineView = false"
      unelevated
      color="white"
      text-color="red-900"
      icon="eva-heart"
    ...
    <q-timeline :layout="layoutValue" :color="color">
      <template v-for="(item, idx) in events" :key="idx">
        ...
        <q-timeline-entry
          ...
          :color="item.entryColor || undefined"
          ...
          <template v-if="item.actions && item.actions.length" #default>
            <div class="q-mt-sm row items-center q-gutter-sm">
              <q-btn
                ...
                dense
                @click="emitAction(action, item, idx)"
              />
            </div>
          </template>
        </q-timeline-entry>
